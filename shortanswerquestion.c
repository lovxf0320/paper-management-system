...
struct shortanswerquestion* creat_short() {      //创建单个选择题
    struct shortanswerquestion* node = (struct shortanswerquestion*)malloc(sizeof(struct shortanswerquestion));
    node->next = node;
    node->prev = node;
    return node;
}

void show_shortquestion(struct shortanswerquestion* node) {               //显示每个选择题
    printf("%s\n", node->name);
    printf("%s\n", node->ans);
...
void choice_one_shortquestion(struct shortanswerquestion* head) {         //查看->选择上一题还是下一题
    int i = -1;
...
struct shortanswerquestion* insert_shortquestion(struct shortanswerquestion* head) {       //管理->选择题->增加
    system("clear");
    ...
    struct shortanswerquestion* rear = head;
...
struct shortanswerquestion* delet_shortquestion(struct shortanswerquestion* head) {        //管理->选择题->删除
    if (head == NULL) {
        printf("链表为空！\n");
        return head;
    }
    ...
    struct shortanswerquestion* new_head = head->next;
...
void admin_one_shortquestion(struct shortanswerquestion* head) {          //管理题目（上一题、下一题、增加、删除）
    int i = -1;
    while (1) {
        printf("                    简答题                    \n");
        if (i == 0)
            ...
            head = save_shortfile(head);
        else if (i == 4)
            head = delet_shortquestion(head);
        system("clear");             //清空终端
        show_shortquestion(head);
...
void load_shortfile(struct shortanswerquestion* head) {
    ...
    FILE* file = fopen("./shortanswerquestion.txt", "r");
    //r只读， w只写（写入模式（清空原内容或创建新文件））， a+(追加模式（在文件末尾添加内容）)，  w+读写（会清空文件）, r+读写
    if (file == NULL)
        return;

    char line[512] = { 0 };
    struct shortanswerquestion* rear = head->prev;

    while (fgets(line, 512, file)) {
        struct shortanswerquestion* node = creat_short();
...
struct shortanswerquestion* save_shortfile(struct shortanswerquestion* head) {
    ...
    FILE* file = fopen("./shortanswerquestion.txt", "a");
    if (file == NULL)
        return head;
