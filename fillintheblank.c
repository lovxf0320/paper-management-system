...
struct blankquestion* creat_blank() {      //创建单个选择题
    struct blankquestion* node = (struct blankquestion*)malloc(sizeof(struct blankquestion));
    node->next = node;
    node->prev = node;
    return node;
}

void show_blankquestion(struct blankquestion* node) {               //显示每个选择题
    printf("%s\n", node->name);
    printf("%s\n", node->ans);
...
void choice_one_blankquestion(struct blankquestion* head) {         //查看->选择上一题还是下一题
    int i = -1;
...
struct blankquestion* insert_blankquestion(struct blankquestion* head) {       //管理->选择题->增加
    system("clear");
    ...
    struct blankquestion* rear = head;
...
struct blankquestion* delet_blankquestion(struct blankquestion* head) {        //管理->选择题->删除
    if (head == NULL) {
        printf("链表为空！\n");
        return head;
    }
    ...
    struct blankquestion* new_head = head->next;
...
void admin_one_blankquestion(struct blankquestion* head) {          //管理题目（上一题、下一题、增加、删除）
    int i = -1;
    while (1) {
        printf("                    选择题                    \n");
        if (i == 0)
            ...
            head = save_blankfile(head);
        else if (i == 4)
            head = delet_blankquestion(head);
        system("clear");             //清空终端
        show_blankquestion(head);
...
void load_blankfile(struct blankquestion* head) {
    ...
    char line[512] = { 0 };
    struct blankquestion* rear =head->prev;

    while (fgets(line, 512, file)) {
        struct blankquestion* node = creat_blank();
